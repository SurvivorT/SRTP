...
class PursuitEvade(AbstractMAEnv, EzPickle):
    ...
    def reset(self):
        ...
        zlb, zub = 0, 1
        constraints = [[xlb, xub], [ylb, yub], [zlb, zub]]
        ...
        self.pursuer_layer = AgentLayer(self.xs, self.ys, self.zs, self.pursuers)

        self.evaders = agent_utils.create_agents(self.n_evaders, self.map_matrix, self.obs_range,
                                                 randinit=True, constraints=constraints)
        self.evader_layer = AgentLayer(self.xs, self.ys, self.zs, self.evaders)
    ...
    def render(self, plt_delay=1.0):
        ...
        for i in range(self.pursuer_layer.n_agents()):
            x, y, z = self.pursuer_layer.get_position(i)
            if z == 0:
        ...
        for i in range(self.evader_layer.n_agents()):
            x, y, z = self.evader_layer.get_position(i)
            if z == 0:
    ...
    def animate(self, act_fn, nsteps, file_name, rate=1.5, verbose=False):
        ...
        for i in range(nsteps):
            ...
            self.save_image(temp_name, cnt = i + 1)
    ...
    def save_image(self, file_name, cnt):
        plt.clf()
        ax1 = plt.subplot(1, 2, 1)
        ax2 = plt.subplot(1, 2, 2)
        plt.title("Frame Number "+str(cnt))
        plt.sca(ax1)
        plt.matshow(self.model_state[0][0].T, cmap=plt.get_cmap('Greys'), fignum=0)
        for i in range(self.pursuer_layer.n_agents()):
            x, y, z = self.pursuer_layer.get_position(i)
            if z == 0:
                plt.plot(x, y, "r*", markersize=12)
                plt.text(x, y, '({},{},{})'.format(x, y, z))
                if self.train_pursuit:
                    ax = plt.gca()
                    ofst = self.obs_range / 2.0
                    ax.add_patch(
        ...
        plt.sca(ax2)
        plt.matshow(self.model_state[0][1].T, cmap=plt.get_cmap('Greys'), fignum=0)
        for i in range(self.pursuer_layer.n_agents()):
            x, y, z = self.pursuer_layer.get_position(i)
            if z == 1:
                plt.plot(x, y, "r*", markersize=12)
    ...
    def reward(self):
        ...
        for i in range(self.n_pursuers):
            idz = self.pursuer_layer.get_position(i)[2]
            # print("split line")
            # print(self.surround_mask[:, 0])
            # print(i)
            # print(np.clip(self.pursuer_layer.get_position(i)[1] + self.surround_mask[:, 1], 0, self.ys - 1))
            # es是evader state
            rewards.append(
            self.catchr * np.sum(
                es[0, np.clip(self.pursuer_layer.get_position(i)[0] + self.surround_mask[:, 0], 0, self.xs - 1),
                   np.clip(self.pursuer_layer.get_position(i)[1] + self.surround_mask[:, 1], 0, self.ys - 1)]
                + es[1, np.clip(self.pursuer_layer.get_position(i)[0] + self.surround_mask[:, 0], 0, self.xs - 1),
                   np.clip(self.pursuer_layer.get_position(i)[1] + self.surround_mask[:, 1], 0, self.ys - 1)])
            )
        # print("rewards:")
        # print(rewards)
    ...
    def collect_obs_by_idx(self, agent_layer, agent_idx):
        ...
        xp, yp, zp = agent_layer.get_position(agent_idx)

        # print("fuck")
        # print(xp, yp)
        xlo, xhi, ylo, yhi, xolo, xohi, yolo, yohi = self.obs_clip(xp, yp)
        # print(self.local_obs.shape)
        # print(self.model_state.shape)
        for idx1 in range(0, 3):
            for idx2 in range(xolo, xohi):
                for idx3 in range(yolo, yohi):
                    self.local_obs[agent_idx][idx1][idx2][idx3] = np.abs(
                    self.model_state[idx1][zp][xlo + idx2 - xolo][ylo + idx3 - yolo]) / self.layer_norm
    ...
    def remove_agents(self):
        ...
        rems = 0
        # 所有pursuier非0状态的位置
        xpur, ypur, zpur = np.nonzero(self.model_state[1])
        purs_sur = np.zeros(self.n_pursuers, dtype=np.bool)
        for i in range(self.n_evaders):
            if self.evaders_gone[i]:
                continue
            x, y, z = self.evader_layer.get_position(ai)
            if self.surround:
                # 周围的位置
                pos_that_catch = self.surround_mask + self.evader_layer.get_position(ai)
                truths = np.array(
                    [np.equal([xi, yi, zi], pos_that_catch).all(axis=1) for xi, yi, zi in zip(xpur, ypur, zpur)])
                if np.sum(truths.any(axis=0)) == self.need_to_surround(x, y, z):
                    #这啥意思
                    ...
                    for j in range(self.n_pursuers):
                        xpp, ypp, zpp = self.pursuer_layer.get_position(j)
    ...
    def need_to_surround(self, x, y, z):
        """
        ...
        if y == 0 or y == (self.ys - 1):
            tosur -= 1
        neighbors = self.surround_mask + np.array([x, y, z])
        for n in neighbors:
            xn, yn, zn = n
            if not 0 < xn < self.xs or not 0 < yn < self.ys:
                continue
            if self.model_state[0][zn, xn, yn] == -1:
                tosur -= 1
        return tosur
    ...
    def idx2state(self, idx):
        ...
        pos = np.unravel_index(idx, [self.xs, self.ys, self.zs] * (self.n_evaders + self.n_pursuers), 'F')
    ...
    def n_states(self):
        # 乘方 map^agent_number
        return (self.xs * self.ys * self.zs) ** (self.n_evaders + self.n_pursuers)
